 ...
 */
public class RectangleGeneralPage extends AttributesUtil.PageWrapper
{
	...
	public void buildUI( Composite parent )
	{
		...
		if ( children != null && children.length > 0 )
		{
			...
			txtText.addFocusListener( new FocusAdapter( ) {
				...
				{
					updateModel( RectangleItem.BAR_CODE );
				};
			} );
			...
			txtAngle.addFocusListener(new FocusAdapter() {
				...
				@Override
				public void focusLost(org.eclipse.swt.events.FocusEvent e) {
					updateModel(RectangleItem.BAR_CODE_TYPE_PROP);
				};
			});
	...
	protected RectangleItem getItem( )
	{
		...
		if ( element instanceof ExtendedItemHandle )
		{
			try
			{
				return (RectangleItem) ( (ExtendedItemHandle) element ).getReportItem( );
			}
			catch ( Exception e )
			{
				e.printStackTrace( );
	...
	private void updateModel( String prop )
	{
		...
		if ( item != null )
		{
			try
			{
				if (RectangleItem.BAR_CODE_TYPE_PROP.equals(prop)) {
					item.setBarCodeType(txtAngle.getText());
				} else if (RectangleItem.BAR_CODE.equals(prop)) {
