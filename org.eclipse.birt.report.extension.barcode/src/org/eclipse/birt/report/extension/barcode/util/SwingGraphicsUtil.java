...
public class SwingGraphicsUtil
{
	...
	public static BufferedImage createRotatedTextImage( String text, int angle,
			Font ft )
	{
		Graphics2D g2d = null;
		try
		{
			...
			BufferedImage stringImage = new BufferedImage( 1,
					1,
	...
	private static BufferedImage createRotatedImage( Object src, int width,
			int height, int angle )
	{
		...
		{
			return renderRotatedObject( src,
					-Math.PI / 2,
					height,
					width,
					-width,
		...
		else if ( angle > 0 && angle < 90 )
		{
			...
			return renderRotatedObject( src, angleInRadians, dW, dH, -width
					* sineTheta
					* sineTheta, width * sineTheta * cosTheta );
		...
		else if ( angle > 180 && angle < 270 )
		{
			...
			return renderRotatedObject( src, angleInRadians, dW, dH, -( width
					* cosTheta * cosTheta ), -( height + width
					* cosTheta
					* sineTheta ) );
